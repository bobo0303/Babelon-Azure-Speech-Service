...
def audio_transcribe(model, audio_file_path, result_queue, ori, stop_event, prev_text=""):
    """  
    Transcribe an audio file, then store the results in a queue.  
    ...
    """  
    transcription_text, rtf, inference_time, language = model.transcribe(audio_file_path, ori, prev_text=prev_text)
    result_queue.put((transcription_text, rtf, inference_time, language))  
    stop_event.set()  # Signal to stop the waiting thread

def audio_translate(model, audio_file_path, result_queue, ori, stop_event, prev_text=""):  
    """  
    ...
    :param stop_event: threading.Event  
        The event used to signal stopping.  
    """  
    transcription_text, translated_text, rtf, translate_time = model.translate(audio_file_path, ori, prev_text=prev_text)  
    result_queue.put((transcription_text, translated_text, rtf, translate_time))  
    stop_event.set()  # Signal to stop the waiting thread  
